...
%     job{k} = batch('Clipper_cov_limb',0,...
%         {input,target,j1,j2,dmin,dmax,Tour_mtker,pon,svfl});
%     pause(.01);

% Modeling Titan Waves
for liq = range
        job{k} = batch('makeWaves_batch',3,{Titan,Titan.liquid.(Composition(liq)),Model,Wind,Uniflow,Etc});% [m]
        pause(.01)
