 ...
 Dimensions of (x,y,k,theta) are (m,n,o,p)
   ...
   Arguments:
       ...
       model
           ...
           cutoff_freq     : cutoff frequency bin separating the diagnostic from advecting wave-numbers
           min_freq        : minimum frequency to model [Hz]
           max_freq        : maximum frequency to model [Hz]
           bathy_map       : m x n array of depth [m] (+ values = subsurface, - values = sub aerial)
...
(2) 'stuck in a while loop calculating wave number. Using less tuned value with error _ERROR_'. The wavenumber is calculated for each frequency by solving iteratively for the function zeros using a Newton-Raphson loop. The default error tolerance to break out of the loop and accept the wavenumber is 1e-4, which can be difficult to reach in a reasonable time for complex bathymetries. This warning allows the model to continue running if the tolerance threshold is not reached in 1e8 iterations for each frequency.
