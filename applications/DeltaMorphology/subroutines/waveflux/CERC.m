...
% ang: angles in degrees
    ...
    p = 0; % dry mass void fraction (what to set this to?)
    ...
    % CERC
    Qs(valid_idx) = K2*(Hnet^(12/5))*(Tnet^(1/5)).*(cos(ang_rad(valid_idx)).^(6/5)) .* sin(ang_rad(valid_idx)); % coefficient * angular component
