...
addpath 'C:\Users\schne\OneDrive\Desktop\Main\Work\MIT\Outside Code\UMWM_Waves'
...
sfcpress = 1.45*[101300];% [Pa].
sfctemp = 91; %273.16 + 23.4; % K.
...
nu = 0.00314/1e4;%1.78e-6;%Liquid viscosity m^2/s
nua = 0.156/1e4;%Gas viscosity m^2/s
sfcT = 0.018; %0.074;% surface tension. N/m
rhow = 456; %1000;% water density.
...
kcg = sqrt(g*rhow/sfcT);% wavenumber of slowest waves -- cap-grav minimum. AGH
kappa = 0.4;
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
%%%Define simulation parameters #1 (Wind Speeds Set Below in #2)
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
maxdelt = 200.0;%Limit of delt to prevent large values as wind ramps up at startup.
...
o = 25;%choose "o" = number of freq. bins; usually ~20.
f1 = 0.05;f2=35;dlnf=(log(f2)-log(f1))/(o-1);%Set frequencies, Titan  %%%SET LIMITS OF SPECTRUM
...
%m=31;n=16; %numxpts=m;numypts=n; % x,y, grid points. Titan


% m=386;n=423; %For input bathymetry map, this has to be the size of the inputer DTM!


p=24; % number of angular (th) bins. p must be factorable by 8 for octants.
...
load Ligeia_smoothed3.mat
D = Sm3;
clear Sm3

[n,m] = size(D); % For input bathymetry map, this is the size of the input DTM
...
% for UU = [ 8 12 20 30]./10;% Run several different wind speeds for fetch laws. Titan
% for UU = [1.25,1.5,1.75,2,2.25,2.5,2.75,3];% Run several different wind speeds for fetch laws. Titan
%for UU = [1.25,1.5,2,2.5,3];% Run several different wind speeds for fetch laws. Titan
...
mss_fac = 120;% MSS adjustment to Sdiss. 2000 produces very satisfactory overshoot, 400 does not. 2009/12/15, Run 986

Snl_fac = 5.0;% fraction of Sdiss that goes to longer wavenumbers.$$$$$$$$$$$$$2010/11/10.
Sds_fac = 4.4596;% This is fac with variable power nnn in Sds.
Sdt_fac = 0.01;
...
for jm = 1:m
    for jn = 1:n
        if D(jm,jn) > 0;
            wn(jm,jn,:) = WAVEKgT(f,D(jm,jn),g,sfcT,rhow,1e-3);% 2013/12/16. %PUT STOP HERE wavelenth = 2*pi/wavenumber
            nnn(jm,jn,:) = 1 + 1.5*(abs(2 - (1+3*(wn(jm,jn,:)./kcg).^2)./(1+(wn(jm,jn,:)./kcg).^2)).^3);% Power of Sds
            ann(jm,jn,:) = Sds_fac.^nnn(jm,jn,:);% Sds_fac for various nnn. %%%%%DESCRIBE DISSIPATION OF SHORT WAVES!!!!!!
        end
    end
end
...
% Delx = 1000.0;% m. Titan
% Dely = 1000.0;% m. Titan
Delx = 3000.0;% m. Titan smoothed by subsampling every 3 points
Dely = 3000.0;% m. Titan
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Added 2011/05/02.
% Calculate renormalization adjusted Snl_arg to determine limits to growth
% in one time step. @@@@@@@@@@@@@@@@@@@@@@@@@@ Added 2011/05/02.
Snl_arg = zeros(m,n,o,p);Snl_arg(:,:,1:end-1,:) = bf1*Snl_fac*(wn(:,:,2:end,:).*dwn(:,:,2:end,:))./(wn(:,:,1:end-1,:).*dwn(:,:,1:end-1,:));
Snl_arg(:,:,1:end-2,:) = Snl_arg(:,:,1:end-2,:) + bf2*Snl_fac*(wn(:,:,3:end,:).*dwn(:,:,3:end,:))./(wn(:,:,1:end-2,:).*dwn(:,:,1:end-2,:));
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Added 2011/05/02.

%########################################################### Added 2010/02/17;
% Set l2 > zref equal to zref;
l2(l2 > zref) = zref;
%########################################################### Added 2010/02/17;
...
if cont == 1;% Read in last step to continue
    t=60;UU=14;
    eval(['load Titan_Ligeia',int2str(file),'_',int2str(t)])
else
    %     E = 0e-4*wn.^(-4);
end
...
Time = 1400; % 10000;% Wind time step
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
%%%Save Initial Calculations
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@


eval(['save Titan_Ligeia_',int2str(file),'_ref m n o p freqs f wn dwn D Uei Uer Cg c delx dely dthd waveang'])
...
for UU=UU;
    ...
    % %######################################### Add random wind
    %       U = U.*(1+0.1*randn(size(U)));% Randomize wind speed
    %       U_z = U;
    % %######################################### Add random wind

    Cd=1.2*ones(size(U)); uj = find(U > 11);Cd(uj)=.49 +.065*U(uj);Cd=Cd/1000;clear uj;% Large and Pond for starters.
    ...
    % Vt = [];
    % Tt = [];
    % AAr = [];%@@@@@@@@@@@@@@@@@ 2010/03/08 to keep track of exponentials.
    % AAn = [];%@@@@@@@@@@@@@@@@@ 2010/03/08 to keep track of exponentials.
    ...
    % ht_maps = zeros(size(E,1),size(E,2),numel(1:Tsteps));
    for t=1:Tsteps
        ...
        while ((Time - sumt) > 0)
            ...
            %
            %     E(2,:,7,8:9) = A1;
            %     E(2,:,8,8:9) = A1*(freqs(1)./freqs(2)).^4;
            %     E(2,:,9,8:9) =  A1*(freqs(1)./freqs(3)).^4;
            %
            % %  Westward Waves added
            %     E(40,:,6,[1 24]) = A1;
            %     E(40,:,7,[1 24]) = A1*(freqs(6)./freqs(7)).^4;
            %     E(40,:,8,[1 24]) =  A1*(freqs(6)./freqs(8)).^4;

            tplot = tplot + 1;
            delt = 0.7 * mindelx / cgmax;% advection limited Courant condition.

            Ul=real(U_z.*sqrt(Cd));Ustar = Ul; U_10 = 2.5*Ul.*log(10/z) + U_z;
            ...
            Ul(D>0)=abs(Ul(D>0).*cos(windir(D>0)-waveang(D>0))-c(D>0)-wfac*UU*cos(windir(D>0)-waveang(D>0))-Uer(D>0).*cth(D>0)-Uei(D>0).*sth(D>0))...
                .*(Ul(D>0).*cos(windir(D>0)-waveang(D>0))-c(D>0)-wfac*UU*cos(windir(D>0)-waveang(D>0))-Uer(D>0).*cth(D>0)-Uei(D>0).*sth(D>0));% Ul is now (Ucos(th)-c-drift*cos(phi)).^2.


            % % Adjust input to lower value when waves overrun the wind.
            Ul = 0.11*Ul;fij=find(Ul<0);Ul(fij) = Ul(fij)*0.005/0.11;clear fij;% Field scale (Donelan et al, 2012). AAAAAAAA
            Sin = zeros(size(Ul));Sin(D>0)=rhorat*(Ul(D>0).*2*pi.*f(D>0).*wn(D>0)./(g+sfcT.*wn(D>0).*wn(D>0)./rhow)) ;
            clear Ul

            %########################################################## Added 2010/02/17.
            % Set Sin = 0 on land boundaries to avoid newdelt --> 0. Use 1e-20 to
            % avoid division by zero in newdelt calculation.
            %@@@@@@@@@@@@@@@@@@@@@ 2010/02/27.
            %  Sin(1,:,:,:) = 1e-20;
            %  Sin(end,:,:,:) = 1e-20;
            %  Sin(:,end,:,:) = 1e-20;
            ...
            % Calculate turbulent dissipation: Sdt = 4 nu_t k^2 = rhoa/rhow*Ustara*7.2*k
            Sdt(:,:,:,:) = repmat(Ustar,[1 1 o p]);clear Ustar
            Sdt = Sdt_fac*sqrt(rhorat).*Sdt.*wn;
            Sbf = zeros(m,n,o,p);
            Sbf(D>0) = 0.01*wn(D>0)./sinh(2*wn(D>0).*D(D>0));
            Sds = zeros(size(Sin));
            Sds(D>0)=abs(ann(D>0)*2*pi.*f(D>0).*((1+mss_fac*short(D>0)).^2.*(wn(D>0).^4.*E(D>0))).^(nnn(D>0))) ;%AAAAAA

            %########################################################## Added 2010/02/17.
            % Set Sds = 0 on land boundaries to avoid newdelt --> 0. Use 1e-20 to
            % avoid division by zero in newdelt calculation.
            %@@@@@@@@@@@@@@@@@@@@@ 2010/02/27.
            Sds(D<=0) = 0;
            %########################################################## Added 2010/02/17.
            ...
            Sds(D>0) = coth(wn(D>0).*D(D>0)).*Sds(D>0) + Sdt(D>0) + Sbf(D>0) + 4*nu*wn(D>0).^2;% Add viscous, turbulent and plunging dissipation after calculation of Snl. 2011/05/16 @@@@@@@@@@@@@@@@
            aa = Sin(:,:,ol,:) + 0.4*Snl_arg(:,:,ol,:).*Sds_wc(:,:,ol,:) - Sds(:,:,ol,:);aa = aa(D(:,:,ol,:)>0);aa = max(aa(:));%$$$ 1.4 gives smooth time-stepping
            aa
            if aa <= 0;aa=0.4/delt;end
            if isnan(aa) == 1;aa = 0.4/maxdelt;end
            newdelt = max([0.4/aa mindelt]);
            ...
            %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            % Keep track of aa exceedance of saa. %@@@@@@@@@@@@@@@@@@@@@@@@@@@ 2010/03/08
            % AAr = [AAr max(aa(:))./saa];
            % jjj = find (aa(:) > saa);
            % AAn = [AAn length(jjj)];
            % Keep track of aa exceedance of saa. %@@@@@@@@@@@@@@@@@@@@@@@@@@@ 2010/03/08
            ...
            E1(:,:,ol,:) = E1(:,:,ol,:) + newdelt*Snl(:,:,ol,:);%Line before and this line subtract Snl and add shifted Snl. 2011/03/30
            cath = ones(size(Sds));cath(D>0)=coth(wn(D>0).*D(D>0));
            E2(:,:,:,:) = zeros(m,n,o,p);fij = find((Sin(:,:,:,:) - 4*nu*wn(:,:,:,:).^2 - Sdt(:,:,:,:) - Sbf(:,:,:,:)) > 0);
            E2(fij) = wn(fij).^(-4).*((Sin(fij)-4*nu*wn(fij).^2 - Sdt(fij) - Sbf(fij))./(cath(fij).*ann(fij)*2*pi.*f(fij))).^(nnninv(fij))./((1+mss_fac*short(fij)).^2);%AAAAAAA
            ...
            if rem(tplot,10) == 0;% plot every 10th time step.
                ...
                % Integrate spectrum  over wavenumber to plot directional plot of 1st wavelength.
                Wavel = sum(squeeze(wn(:,:,1,:)).*squeeze(E(:,:,1,:).*dwn(:,:,1,:)),3)./sum(squeeze(E(:,:,1,:).*dwn(:,:,1,:)),3);
                Wavel = 2*pi./Wavel;Wavel = Wavel.^(0.25);% Wavelength to 1/4 power.
                % Direction of one wavelength.
                KD = squeeze(sum(dwn(:,:,1,:).*E(:,:,1,:),3));
                Dir_sin = sum(KD.*squeeze(sin(waveang(:,:,1,:))),3)./sum(KD,3);
                Dir_cos = sum(KD.*squeeze(cos(waveang(:,:,1,:))),3)./sum(KD,3); mdir = atan2(Dir_sin,Dir_cos);
                % end direction of one wavelength.
                %%%figure(20);hold on;quiver(Wavel'.*cos(mdir'),Wavel'.*sin(mdir'),0.8,'c');
            ...
            cgmax = max(max(max(max((E>1e-20).*Cg))));% Adjust cgmax for active energy components.
...
%         fraction_time_completed = t/TstepsU
        fraction_time_completed = t/Tsteps % Taylor fixed bug?
        ttime=toc;
        seconds_so_far = toc
        hours_to_complete = Tsteps/t*ttime/3600

        %save output
        %savefile = ['Titan_LigeiaOpp_',num2str(round(mean(U_orig(:)).*10)./10),'mps_',int2str(file),'_',int2str(round(Time.*t)),'s.mat'];
        eval(['save Titan_LigeiaOpp_',num2str(round(mean(U_orig(:)).*10)./10),'mps_',int2str(file),'_',int2str(round(Time.*t)),'s.mat',' E ht freqs Cd Sds Sds_wc Sin Snl Sdt Newdelt windir U_orig'])
       
        %send a text message saying that we successfully output a run
        %send_text_message('6077937531','verizon','waves code',['Finished ' savefile]);
    end
