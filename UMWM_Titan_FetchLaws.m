...
kappa = 0.4; % Von-Karman constant -- UGS
...
% make bathymetry surface-ish plot -- UGS
if ploton;
    figure(1);
    clf;  %clear figure
    pcolor(D);
...
%% HERE THE MODEL STARTS CALCULATING STUFF -- UGS
wn(:,:,:) = ones(m,n,o);
nnn(:,:,:) = ones(m,n,o);
ann(:,:,:) = ones(m,n,o);
for jm = 1:m
...
if cont == 1;% Read in last step to continue (ignore this for now, maybe get rid of this functionality? throw in an outside function?) -- UGS
    t=60;
    UU=14;
