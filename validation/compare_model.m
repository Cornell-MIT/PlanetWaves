...
% compares PlanetWave results of wave height for a given wind speed to UMWM
% Pierson-Moskowitz, and JONSWAP for deepwater buoy observations in Lake
% Superior 

addpath(genpath(fullfile('..','data','Earth')))
addpath(genpath(fullfile('..','validation','Earth')))
...
% criteria for wind selection
wind_variability = 3;                                                      % varies by less than u_t m/s
wind_dir_variability = 15;                                                 % varies by less than dir_t degrees
gusty_variability = 1.5;                                                   % gustiness varies by less than g_t*100 %


[u,h,angle] = table_quiet_times(wind_variability,wind_dir_variability,gusty_variability);
...
figure('units', 'normalized', 'outerposition', [0 0 1 1],'Name','Waves at Lake Superior w Models');
fax1 = axes;
p1 = scatter(fax1,Events.U,Events.obs_H,150,Events.F/1000,'filled'); % buoy observations
cb = colorbar;
colormap(mymap)
...
p2 = plot(fax1,test_speeds, JS, '--','Color',[0.7 0.7 0.7], 'LineWidth', 3,'HandleVisibility','off');
p4 = fill(fax1,[test_speeds, fliplr(test_speeds)], [JS_p, fliplr(JS_m)], 'k', 'FaceAlpha', 0.3, 'EdgeAlpha', 0,'HandleVisibility','off');
p5 = fill(fax1,[test_speeds, fliplr(test_speeds)], [JS_m, fliplr(JS_p)], 'k', 'FaceAlpha', 0.3, 'EdgeAlpha', 0,'HandleVisibility','off');
...
wavetable = readtable('umwm_wind_waveheights.xlsx'); % table containing wind speed | UMWM wave height | PlanetWaves wave height
...
legend([p1 p8 p2 p6 p7],'Observation','Pierson-Moskowitz','JONSWAP','UMWM','PlanetWaves','Location','best')
%exportgraphics(gcf, 'LSCompare.png', 'ContentType', 'vector');

figure('units', 'normalized', 'outerposition', [0 0 1 1],'Name','R-squared of PlanetWaves vs UMWM');
